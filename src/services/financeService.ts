...
// // ✅ Update Transaction + Adjust Budget
// export const updateTransaction = async (id: string, data: Partial<Transaction>) => {
//   try {
//     const oldTx = await databases.getDocument(DB_ID, COLLECTION_TRANSACTIONS, id);
//     const updatedTx = await databases.updateDocument(DB_ID, COLLECTION_TRANSACTIONS, id, data);

//     const month = (data.date || oldTx.date).slice(0, 7);
//     const category = data.category || oldTx.category;
...
//     if (budgets.documents.length > 0) {
//       const budget = budgets.documents[0];
//       let newSpent = budget.spentAmount || 0;

//       if (oldTx.type === "expense") newSpent -= oldTx.amount;
//       if (data.type === "expense" || oldTx.type === "expense")
//         newSpent += data.amount ?? oldTx.amount;

//       await databases.updateDocument(DB_ID, COLLECTION_BUDGETS, budget.$id, {
//         spentAmount: Math.max(0, newSpent),
//       });
//     }
...
// // ✅ Delete Transaction + Adjust Budget
// export const deleteTransaction = async (transactionId: string) => {
//   try {
//     const tx = await databases.getDocument(DB_ID, COLLECTION_TRANSACTIONS, transactionId);
//     const month = tx.date.slice(0, 7);
...
//     if (budgets.documents.length > 0 && tx.type === "expense") {
//       const budget = budgets.documents[0];
//       const updatedSpent = Math.max((budget.spentAmount || 0) - tx.amount, 0);
//       await databases.updateDocument(DB_ID, COLLECTION_BUDGETS, budget.$id, {
//         spentAmount: updatedSpent,
//       });
//     }
...
// // ✅ Create or Merge Budget + Sync Transaction
// export async function createBudget(budget: Budget) {
//   try {
//     const existing = await databases.listDocuments(DB_ID, COLLECTION_BUDGETS, [
//       Query.equal("userId", budget.userId),
...
//     if (existing.documents.length > 0) {
//       const doc = existing.documents[0];
//       const updatedBudgetAmount = (doc.budgetAmount || 0) + budget.budgetAmount;
//       const updatedSpentAmount = (doc.spentAmount || 0) + (budget.spentAmount || 0);
...
//     // 🔁 Create expense transaction if spentAmount > 0
//     if (budget.spentAmount && budget.spentAmount > 0) {
...
//       await databases.createDocument(DB_ID, COLLECTION_TRANSACTIONS, ID.unique(), transaction);
//     }

//     return budgetId;
//   } catch (error) {
//     console.error("Error creating/merging budget:", error);
//     throw new Error("Failed to create or merge budget");
//   }
// }
